...
```
## Hook
- Word count: [CON SỐ CỤ THỂ] từ
- Formula: [Formula ID đã chọn, VD: F15]
- Hook Intent: [Ý cốt lõi cần truyền tải — diễn giải bằng lời, KHÔNG copy nguyên văn Core Hook]
- Core Hook (nguyên liệu thô — Voice Writer sẽ viết lại bằng Voice DNA): [Câu hook từ 03-hook.md]
- Atoms: [Atom ID/loại]
- VTS: [Value signal cụ thể]
- [Nội dung outline cho phần mở rộng sau câu hook]

## [Tên Section — Story/Deep Dive/Pivot]
...
⛔ **Section Hook**: CẤM copy nguyên văn Core Hook từ `03-hook.md` làm chỉ thị trong outline. Phải tách thành `Hook Intent` (diễn giải ý) và `Core Hook` (nguyên liệu thô có gắn nhãn).
