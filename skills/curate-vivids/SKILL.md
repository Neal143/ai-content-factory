...
```powershell
python .agents/skills/curate-vivids/scripts/extract_vivids.py "[cache_file]" --output "[run_folder]/vivid_candidates.json" --split-dir "[run_folder]/vivid_chunks"
```
...
```powershell
python .agents/skills/curate-vivids/scripts/extract_vivids.py --session-dir "[run_folder]/vivid_chunks" --get-next
```
> Script xuất file `[run_folder]/current_batch.json`. Dùng `view_file` đọc file này để lấy nội dung batch và `batch_password`.
...
3. **Tạo file Bảng điểm** `[run_folder]/eval_temp.json` với cấu trúc CHÍNH XÁC sau:
```json
{
...
4. **Nộp Bài:**
```powershell
python .agents/skills/curate-vivids/scripts/extract_vivids.py --session-dir "[run_folder]/vivid_chunks" --submit-file "[run_folder]/eval_temp.json"
```
